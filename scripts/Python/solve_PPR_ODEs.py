...
if __name__ == "__main__":
    ...
    nt = 100000  # numbser of time steps
    ...
    for coupling_condition in coupling_conditions:
        ...
        data_directory = os.path.join("../../data/Paired_Pulse_data/" + coupling_condition + "/")
        ...
        for cell_condition in cell_conditions:
            ...
            for vgcc_channel_number in vgcc_channel_number_range:
                ...
                DualSensorModel_PPR = pd.DataFrame(index=list(range(1, num_trials+1)), columns=[1]) #Paired Pulse Ratio values dimension = (Number of trails, 1)

                DualSensorModel_released_vesicles_stim_1 = pd.DataFrame(index=list(range(1, num_trials + 1)), columns=list(np.arange(1, 30000 + 1)))  # Total vesicles released
                DualSensorModel_released_vesicles_stim_2 = pd.DataFrame(index=list(range(1, num_trials + 1)), columns=list(np.arange(1, 30000 + 1)))  # Total vesicles released
                DualSensorModel_RRV_stim_1 = pd.DataFrame(index=list(range(1, num_trials + 1)), columns=list(np.arange(1, 30000 + 1)))  # Total Vesicles in the Release Ready Pool (i,e FRP and SRP)
                DualSensorModel_RRV_stim_2 = pd.DataFrame(index=list(range(1, num_trials + 1)), columns=list(np.arange(1, 30000 + 1)))  # Total Vesicles in the Release Ready Pool (i,e FRP and SRP)
                DualSensorModel_release_proba_stim_1 = pd.DataFrame(index=list(range(1, num_trials + 1)), columns=list(np.arange(1, 30000 + 1)))  # Vesicle release probability
                DualSensorModel_release_proba_stim_2 = pd.DataFrame(index=list(range(1, num_trials + 1)), columns=list(np.arange(1, 30000 + 1)))  # Vesicle release probability
                ...
                for trial in range(1, num_trials+1):
                    ...
                    [PoPQ, PoIPR, IcaPQ_type, time, y] = solve_ODEs(time_step, init, release_params, IP3Receptor_params,
                                                               ...
                                                               ISI, coupling_condition, dT_save, AP_type="Single",
...
### %%%%%%%%%%%%%%%# Dual Sensor Release Rates %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    ...
                    DualSensorModel_released_vesicles_stim_1.loc[trial, :] = scipy.integrate.cumtrapz(time[0:30000],
                                                                            DualSensorModel_release_rate.loc[trial, 1:30000])

                    DualSensorModel_released_vesicles_stim_2.loc[trial, :] = scipy.integrate.cumtrapz(time[43999:73998],
                                                                            DualSensorModel_release_rate.loc[trial, 44000:73999])
                    num_vesicles_in_FRP = 5
                    num_vesicles_in_SRP = 5
                    DualSensorModel_release_proba_stim_1.loc[trial, :] = scipy.integrate.cumtrapz(time[0:30000],
                                                                                          DualSensorModel_SlowRelRate.loc[trial,1:30000]/ num_vesicles_in_SRP\
                                                                                        + DualSensorModel_FastRelRate.loc[trial,1:30000]/num_vesicles_in_FRP)
                    DualSensorModel_release_proba_stim_2.loc[trial, :] = scipy.integrate.cumtrapz(time[43999:73998],
                                                                                          DualSensorModel_SlowRelRate.loc[trial,44000:73999]/ num_vesicles_in_SRP\
                                                                                        + DualSensorModel_FastRelRate.loc[trial,44000:73999]/num_vesicles_in_FRP)

                    DualSensorModel_PPR.loc[trial, :] = DualSensorModel_release_proba_stim_2.loc[trial, 30000]/\
                                                        DualSensorModel_release_proba_stim_1.loc[trial, 30000]
                    ...
                    DualSensorModel_RRV_stim_1.loc[trial, :] = DualSensorModel_Slow_RRV[1:30000] + DualSensorModel_Fast_RRV[1:30000]
                    DualSensorModel_RRV_stim_2.loc[trial, :] = DualSensorModel_Slow_RRV[44000:73999] + DualSensorModel_Fast_RRV[44000:73999]
                ...
                filename  = f"{data_directory}/{cell_condition}_PPR_Rate_ReservePool_{N_vgcc}_VGCC.txt"
                DualSensorModel_Rate_ReservePool.mean(axis=0).to_csv(filename, header=False, index=False)

                filename =  f"{data_directory}/{cell_condition}_PPR_Rate_DockedPool_{N_vgcc}_VGCC.txt"
                DualSensorModel_Rate_DockedPool.mean(axis=0).to_csv(filename, header=False, index=False)

                filename = f"{data_directory}/{cell_condition}_PPR_AsyncRelRate_{N_vgcc}_VGCC.txt"
                DualSensorModel_AsyncRelRate.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_PPR_SyncRelRate_{N_vgcc}_VGCC.txt"
                DualSensorModel_SyncRelRate.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_PPR_SpontRelRate_{N_vgcc}_VGCC.txt"
                DualSensorModel_SpontRelRate.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_PPR_SlowRelRate_{N_vgcc}_VGCC.txt"
                DualSensorModel_SlowRelRate.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  =  f"{data_directory}/{cell_condition}_PPR_FastRelRate_{N_vgcc}_VGCC.txt"
                DualSensorModel_FastRelRate.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_PPR_RelVes_stim1_{N_vgcc}_VGCC.txt"
                DualSensorModel_released_vesicles_stim_1.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_PPR_RelVes_stim2_{N_vgcc}_VGCC.txt"
                DualSensorModel_released_vesicles_stim_2.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_PPR_RelRate_{N_vgcc}_VGCC.txt"
                DualSensorModel_release_rate.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_PPR_RelProba_stim1_{N_vgcc}_VGCC.txt"
                DualSensorModel_release_proba_stim_1.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_PPR_RelProba_stim2_{N_vgcc}_VGCC.txt"
                DualSensorModel_release_proba_stim_2.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_PPR_RRV_stim1_{N_vgcc}_VGCC.txt"
                DualSensorModel_RRV_stim_1.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_PPR_RRV_stim2_{N_vgcc}_VGCC.txt"
                DualSensorModel_RRV_stim_2.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_PPR_Ratio_{N_vgcc}_VGCC.txt"
                DualSensorModel_PPR.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_PPR_IP3_Calcium_{N_vgcc}_VGCC.txt"
                IP3R_Calcium.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_PPR_VGCC_Calcium_{N_vgcc}_VGCC.txt"
                VGCC_Calcium.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_PPR_CYTO_Calcium_{N_vgcc}_VGCC.txt"
