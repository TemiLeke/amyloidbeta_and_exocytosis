...
if __name__ == "__main__":
    ...
    for coupling_condition in coupling_conditions:
        ...
        data_directory = os.path.join("../../data/SingleAP_data/" + coupling_condition + "/")
        ...
        for cell_condition in cell_conditions:
            ...
            for vgcc_channel_number in vgcc_channel_number_range:
                ...
                DualSensorModel_release_proba = pd.DataFrame(index=list(range(1, num_trials+1)), columns=list(np.arange(1, nt+1))) # Vesicle release probability
...
### %%%%%%%%%%%%%%%% Dual Sensor Release Rates %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    ...
                    DualSensorModel_release_proba.loc[trial, :] = scipy.integrate.cumtrapz(time, DualSensorModel_SlowRelRate.loc[trial,:]/num_vesicles_in_SRP\
                                                                                            + DualSensorModel_FastRelRate.loc[trial,:]/num_vesicles_in_FRP)
                ...
                filename  = f"{data_directory}/{cell_condition}_SingleAP_Rate_ReservePool_{N_vgcc}_VGCC.txt"
                DualSensorModel_Rate_ReservePool.mean(axis=0).to_csv(filename, header=False, index=False)

                filename =  f"{data_directory}/{cell_condition}_SingleAP_Rate_DockedPool_{N_vgcc}_VGCC.txt"
                DualSensorModel_Rate_DockedPool.mean(axis=0).to_csv(filename, header=False, index=False)

                filename = f"{data_directory}/{cell_condition}_SingleAP_AsyncRelRate_{N_vgcc}_VGCC.txt"
                DualSensorModel_AsyncRelRate.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_SingleAP_SyncRelRate_{N_vgcc}_VGCC.txt"
                DualSensorModel_SyncRelRate.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_SingleAP_SpontRelRate_{N_vgcc}_VGCC.txt"
                DualSensorModel_SpontRelRate.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_SingleAP_SlowRelRate_{N_vgcc}_VGCC.txt"
                DualSensorModel_SlowRelRate.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  =  f"{data_directory}/{cell_condition}_SingleAP_FastRelRate_{N_vgcc}_VGCC.txt"
                DualSensorModel_FastRelRate.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_SingleAP_RelVes_{N_vgcc}_VGCC.txt"
                DualSensorModel_released_vesicles.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_SingleAP_RelRate_{N_vgcc}_VGCC.txt"
                DualSensorModel_release_rate.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_SingleAP_RelProba_{N_vgcc}_VGCC.txt"
                DualSensorModel_release_proba.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_SingleAP_RRV_{N_vgcc}_VGCC.txt"
                DualSensorModel_RRV.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_SingleAP_IP3_Calcium_{N_vgcc}_VGCC.txt"
                IP3R_Calcium.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_SingleAP_VGCC_Calcium_{N_vgcc}_VGCC.txt"
                VGCC_Calcium.mean(axis=0).to_csv(filename, header=False, index=False)

                filename  = f"{data_directory}/{cell_condition}_SingleAP_CYTO_Calcium_{N_vgcc}_VGCC.txt"
