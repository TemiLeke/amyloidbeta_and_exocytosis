...
for index=1:length(ca_source)
    ...
    data_directory = strcat("../../data/SingleAP_data_scl/", calcium_source, "/");
    mkdir(data_directory)
    
    sub_con_level = 1;                          % ms    subconductance level
    n_mot_percent_incr = 100;         % average percentage increase in SCL         
    ...
    for time_of_recording=20:20:80      % time of recording in minutes.
        ...
        for k=35     % Interation over Number of channels ranging from 5 to 150 VGCC number
            ...
            DualSensorModel_release_proba = zeros(num_trials, time_steps);              % Vesicle release probability 
            ...
            Abeta_Calcium = zeros(num_trials, time_steps);      % Calcium concentration in Sub-plasmalemmal Amyloid beta compartment
            ...
            for n=1:num_trials
                ...
                N_pores = 200;                      % number of abeta pores at recording time
                SCL = sub_con_level;                % Subconductance level at recording time
                ...
                mean_open_time = 10;                                % ms    Mean open duration of the pore at time of recording
            ...
            % Reccruitment model parameters |   Unit    |   Description
                ...
                DualSensorModel_release_proba(n, :) = cumtrapz(t, DualSensorModel_Slow(1, :)./num_vesicles_in_SRP...
                                                        + DualSensorModel_Fast(1, :)./num_vesicles_in_FRP);
                ...
                DualSensorModel_RRV(n, :) = DualSensorModel_Slow_RRV + DualSensorModel_Fast_RRV;
                ...
                DualSensorModel_Cummulative = cumsum(DualSensorModel);
                ...
                filename  = sprintf("SingleAP_Rate_ReservePool_%d_min.txt", time_of_recording);
                fileID = fopen(strcat(data_directory, filename), "w");
                fprintf(fileID, "%.15f\n", mean(DualSensorModel_Rate_ReservePool, 1));
                fclose(fileID);

                filename  = sprintf("SingleAP_Rate_DockedPool_%d_min.txt", time_of_recording);
                fileID = fopen(strcat(data_directory, filename), "w");
                fprintf(fileID, "%.15f\n", mean(DualSensorModel_Rate_DockedPool, 1));
                fclose(fileID);

                filename  = sprintf("SingleAP_AsyncRelRate_%d_min.txt", time_of_recording);
                fileID = fopen(strcat(data_directory, filename), "w");
                fprintf(fileID, "%.15f\n", mean(DualSensorModel_AsyncRelRate, 1));
                fclose(fileID);

                filename  = sprintf("SingleAP_SyncRelRate_%d_min.txt", time_of_recording);
                fileID = fopen(strcat(data_directory, filename), "w");
                fprintf(fileID, "%.15f\n", mean(DualSensorModel_SyncRelRate, 1));
                fclose(fileID);

                filename  = sprintf("SingleAP_SpontRelRate_%d_min.txt", time_of_recording);
                fileID = fopen(strcat(data_directory, filename), "w");
                fprintf(fileID, "%.15f\n", mean(DualSensorModel_SpontRelRate, 1));
                fclose(fileID);

                filename  = sprintf("SingleAP_SlowRelRate_%d_min.txt", time_of_recording);
                fileID = fopen(strcat(data_directory, filename), "w");
                fprintf(fileID, "%.15f\n", mean(DualSensorModel_SlowRelRate, 1));
                fclose(fileID);

                filename  = sprintf("SingleAP_FastRelRate_%d_min.txt", time_of_recording);
                fileID = fopen(strcat(data_directory, filename), "w");
                fprintf(fileID, "%.15f\n", mean(DualSensorModel_FastRelRate, 1));
                fclose(fileID);

                filename  = sprintf("SingleAP_RelVes_%d_min.txt", time_of_recording);
                fileID = fopen(strcat(data_directory, filename), "w");
                fprintf(fileID, "%.15f\n", mean(DualSensorModel_released_vesicles, 1));
                fclose(fileID);

                filename  = sprintf("SingleAP_RelRate_%d_min.txt", time_of_recording);
                fileID = fopen(strcat(data_directory, filename), "w");
                fprintf(fileID, "%.15f\n", mean(DualSensorModel_release_rate, 1));
                fclose(fileID);

                filename  = sprintf("SingleAP_RelProba_%d_min.txt", time_of_recording);
                fileID = fopen(strcat(data_directory, filename), "w");
                fprintf(fileID, "%.15f\n", mean(DualSensorModel_release_proba, 1));
                fclose(fileID);

                filename  = sprintf("SingleAP_RRV_%d_min.txt", time_of_recording);
                fileID = fopen(strcat(data_directory, filename), "w");
                fprintf(fileID, "%.15f\n", mean(DualSensorModel_RRV, 1));
                fclose(fileID);

                filename  = sprintf("SingleAP_IP3_Calcium_%d_min.txt", time_of_recording);
                fileID = fopen(strcat(data_directory, filename), "w");
                fprintf(fileID, "%.15f\n", mean(IP3R_Calcium, 1));
                fclose(fileID);

                filename  = sprintf("SingleAP_VGCC_Calcium_%d_min.txt", time_of_recording);
                fileID = fopen(strcat(data_directory, filename), "w");
                fprintf(fileID, "%.15f\n", mean(VGCC_Calcium, 1));
                fclose(fileID);

                filename  = sprintf("SingleAP_CYTO_Calcium_%d_min.txt", time_of_recording);
                fileID = fopen(strcat(data_directory, filename), "w");
                fprintf(fileID, "%.15f\n", mean(Cyto_Calcium, 1));
                fclose(fileID);

                filename  = sprintf("SingleAP_ABETA_Calcium_%d_min.txt", time_of_recording);
                fileID = fopen(strcat(data_directory, filename), "w");
                fprintf(fileID, "%.15f\n", mean(Abeta_Calcium, 1));
                fclose(fileID);
        end
        sub_con_level = sub_con_level + sub_con_level;                                    % ms    save MOT for next iteration/time of recording        
    end
end