...
for index=1:length(ca_source)
    ...
    data_directory = strcat("../../data/PairedPulse_data_scl/", calcium_source, "/");
    mkdir(data_directory)

    sub_con_level = 1;                % ms    Mean open duration of the pore at 20 minutes
    ...
    for time_of_recording=20:20:80      % time of recording in minutes.
        ...
        %% Saving Pre-Data
            ...
            filename  = sprintf("PPR_Rate_ReservePool_%d_min.txt", time_of_recording);
            fileID = fopen(strcat(data_directory, filename), "w");
            fprintf(fileID, "%.15f\n", mean(DualSensorModel_Rate_ReservePool, 1));
            fclose(fileID);

            filename  = sprintf("PPR_Rate_DockedPool_%d_min.txt", time_of_recording);
            fileID = fopen(strcat(data_directory, filename), "w");
            fprintf(fileID, "%.15f\n", mean(DualSensorModel_Rate_DockedPool, 1));
            fclose(fileID);

            filename  = sprintf("PPR_AsyncRelRate_%d_min.txt", time_of_recording);
            fileID = fopen(strcat(data_directory, filename), "w");
            fprintf(fileID, "%.15f\n", mean(DualSensorModel_AsyncRelRate, 1));
            fclose(fileID);

            filename  = sprintf("PPR_SyncRelRate_%d_min.txt", time_of_recording);
            fileID = fopen(strcat(data_directory, filename), "w");
            fprintf(fileID, "%.15f\n", mean(DualSensorModel_SyncRelRate, 1));
            fclose(fileID);

            filename  = sprintf("PPR_SpontRelRate_%d_min.txt", time_of_recording);
            fileID = fopen(strcat(data_directory, filename), "w");
            fprintf(fileID, "%.15f\n", mean(DualSensorModel_SpontRelRate, 1));
            fclose(fileID);

            filename  = sprintf("PPR_SlowRelRate_%d_min.txt", time_of_recording);
            fileID = fopen(strcat(data_directory, filename), "w");
            fprintf(fileID, "%.15f\n", mean(DualSensorModel_SlowRelRate, 1));
            fclose(fileID);

            filename  = sprintf("PPR_FastRelRate_%d_min.txt", time_of_recording);
            fileID = fopen(strcat(data_directory, filename), "w");
            fprintf(fileID, "%.15f\n", mean(DualSensorModel_FastRelRate, 1));
            fclose(fileID);

            filename  = sprintf("PPR_RelRate_%d_min.txt", time_of_recording);
            fileID = fopen(strcat(data_directory, filename), "w");
            fprintf(fileID, "%.15f\n", mean(DualSensorModel_release_rate, 1));
            fclose(fileID);

            filename  = sprintf("PPR_RelVes_stim1_%d_min.txt", time_of_recording);
            fileID = fopen(strcat(data_directory, filename), "w");
            fprintf(fileID, "%.15f\n", mean(DualSensorModel_released_vesicles_stim_1, 1));
            fclose(fileID);

            filename  = sprintf("PPR_RelVes_stim2_%d_min.txt", time_of_recording);
            fileID = fopen(strcat(data_directory, filename), "w");
            fprintf(fileID, "%.15f\n", mean(DualSensorModel_released_vesicles_stim_2, 1));
            fclose(fileID);

            filename  = sprintf("PPR_RelProba_stim1_%d_min.txt", time_of_recording);
            fileID = fopen(strcat(data_directory, filename), "w");
            fprintf(fileID, "%.15f\n", mean(DualSensorModel_release_proba_stim_1, 1));
            fclose(fileID);

            filename  = sprintf("PPR_RelProba_stim2_%d_min.txt", time_of_recording);
            fileID = fopen(strcat(data_directory, filename), "w");
            fprintf(fileID, "%.15f\n", mean(DualSensorModel_release_proba_stim_2, 1));
            fclose(fileID);

            filename  = sprintf("PPR_RRV_stim1_%d_min.txt", time_of_recording);
            fileID = fopen(strcat(data_directory, filename), "w");
            fprintf(fileID, "%.15f\n", mean(DualSensorModel_RRV_stim_1, 1));
            fclose(fileID);

            filename  = sprintf("PPR_RRV_stim2_%d_min.txt", time_of_recording);
            fileID = fopen(strcat(data_directory, filename), "w");
            fprintf(fileID, "%.15f\n", mean(DualSensorModel_RRV_stim_2, 1));
            fclose(fileID);

            filename  = sprintf("PPR_Ratio_%d_min.txt", time_of_recording);
            fileID = fopen(strcat(data_directory, filename), "w");
            fprintf(fileID, "%.15f\n", mean(DualSensorModel_PPR, 1));
            fclose(fileID);


            filename  = sprintf("PPR_IP3_Calcium_%d_min.txt", time_of_recording);
            fileID = fopen(strcat(data_directory, filename), "w");
            fprintf(fileID, "%.15f\n", mean(IP3R_Calcium, 1));
            fclose(fileID);

            filename  = sprintf("PPR_VGCC_Calcium_%d_min.txt", time_of_recording);
            fileID = fopen(strcat(data_directory, filename), "w");
            fprintf(fileID, "%.15f\n", mean(VGCC_Calcium, 1));
            fclose(fileID);

            filename  = sprintf("PPR_CYTO_Calcium_%d_min.txt", time_of_recording);
            fileID = fopen(strcat(data_directory, filename), "w");
            fprintf(fileID, "%.15f\n", mean(Cyto_Calcium, 1));
            fclose(fileID);
        ...
        sub_con_level = sub_con_level + sub_con_level;                                    % ms    save sub_con_level for next iteration/time of recording        
    end
end